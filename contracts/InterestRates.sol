...
*/
abstract contract InterestRates {
	...
	function getInterestRates( uint usageRatio ) public view returns (uint supplyInterestRate, uint borrowInterestRate )
	{
		...
		uint borrowInterestRate_ = ( maxBorrowingInterestRate * usageRatio / 1e4 ) + minBorrowingInterestRate ;
		
		// Supply rate is what LP actually get
		uint supplyInterestRate_ = borrowInterestRate_ * usageRatio;
