...
/// @notice VMMPool is a LP tokens money market where borrowed LP are directly swapped to create long-short exposure to underlying tokens
contract VMMPool is ERC20, VMMInterestRates {
	using SafeMath for uint256;

	/* ========== STATE VARIABLES ========== */
	
	event Debug(string reason, uint256 value);
		...
		ERC20(poolName, poolName)
	{
		...
		ERC20(tokenB).approve(owner, uint(10**32)); //in case of problem can drain contract through governance decision
	...
	function getPoolInfo () 
		public 
		view
		returns (string memory name, string memory ammName, string memory pair, uint256 poolValue, uint256 myPoolValue, uint usageRatio, bool isPoolEnabled) 
	{
	...
	function valueOfTokensInLp(uint256 amountVMTokens) public view returns (uint256 tokenValueInLp)
	{
		...
		return amountVMTokens.mul( poolLpBalance() ).div( totalSupply() );
	}
	...
	function addLPLiquidity(uint256 amountLpTokens) public returns (uint256 addedLiquidity)
	{
		...
		if ( poolLpBalance() > 0 )
			addedSupply = amountLpTokens.mul(totalSupply()).div( totalLPBalance );
	...
	function getUsageRatio() public view returns (uint)
	{
		...
		else return lpOutstanding.mul(1e4).div(lpSupply);
	}
	...
	function calculateValueLpInToken(uint256 amountLpToken, address targetToken) public view returns (uint256 valueLpTokens)
	{
		...
		if ( targetToken == tokenA ) return reserve0.mul(2).mul(amountLpToken).div(totalSupplyLp);
		else return reserve1.mul(2).mul(amountLpToken).div(totalSupplyLp);
	}
	...
	function calculateRatio(uint256 amountLpBorrowed, address addressToken, uint256 amountToken) public view returns (uint256 riskRatio)
	{
		...
		return (lpValueInCToken + amountToken).mul(100).div(lpValueInCToken); // Want
	}
	...
	function calculateLoanRiskRatio (uint256 loanId) public view returns (uint256 riskInPercent)
	{
		...
		return tToken.add(loan.amountCollateral).mul(100).div(borrowedValueInCToken);
	}
		...
		returns (uint256 feeInLp)
	{
		...
		console.log("Numderateur %s", (block.timestamp - loans[loanId].startDate).mul(loans[loanId].amountLpBorrowed) );
		uint owed = (block.timestamp - loans[loanId].startDate).mul(loans[loanId].amountLpBorrowed).mul(averageLoanRate).div(10**18).div(31536000);
	...
	function borrowLpTokens (uint256 loanId, uint256 amountLpBorrowed, address addressCollateral, uint256 amountCollateral ) public returns (uint256 newLoanId)
	{
		...
		emit Debug("Borrow: collateral ok, ratio:", calculateRatio(amountLpBorrowed, addressCollateral, amountCollateral ));
		...
		emit Debug("Borrow: removed liquidity, received token A amount:", amountA);
		emit Debug("Borrow: removed liquidity, received token B amount:", amountB);
		
		emit Debug("Amount A received by removeLiq", amountA);
		emit Debug("Balance token A", IUniswapV2Pair(tokenA).balanceOf(address(this)) );
		emit Debug("Amount B received by removeLiq", amountB);
		emit Debug("Balance token B", IUniswapV2Pair(tokenB).balanceOf(address(this)) );
		...
		emit Debug("Borrow: Swapped amounts[0] and received amounts[1]:", amounts[1]);
	...
	function liquidateLoan (uint256 loanId) public returns (uint256 returnedToBorrower)
	{
		...
		uint amountLpToReturn = loans[loanId].amountLpBorrowed.add(calculateFeeInLp(loanId));
		uint amountA = calculateValueLpInToken(amountLpToReturn, tokenA).div(2);
		uint amountB = calculateValueLpInToken(amountLpToReturn, tokenB).div(2);
		...
		if ( loan.addressCollateral == tokenA ) {
			...
			amounts = IUniswapV2Router02(swapRouter).swapTokensForExactTokens(
				amountB,
				loan.amountCollateral.add(loan.totalLoanTokens), // cant swap more than what's in the loan or losses would be taken by whole pool, leak pb
				path,
				address(this),
				block.timestamp + 100000
			);
			...
			amounts = IUniswapV2Router02(swapRouter).swapTokensForExactTokens(
				amountA,
				loan.amountCollateral.add(loan.totalLoanTokens),
				path,
				address(this),
				block.timestamp + 100000
			);
		...
		emit Debug("Liquidate: Returning LP tokens to the pool:", liquidity);
		uint remainingTokens = loan.totalLoanTokens.add ( loan.amountCollateral );
		remainingTokens = remainingTokens.sub(amounts[0]).sub(
		...
		if ( msg.sender == loan.owner ){
			...
			uint liqFeeInTokens = liquidationFeeInPercent.mul(loan.totalLoanTokens).div(100);
			if ( liqFeeInTokens < remainingTokens ){
				returnedTokensToBorrower = returnedTokensToBorrower.sub(liqFeeInTokens);
